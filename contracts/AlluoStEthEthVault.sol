...
contract AlluoStEthEthVault is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, ERC4626Upgradeable {
 ...
 /// @notice Stakes all underlying LP tokens that are not already staked. 
    ...
    function depositWithoutLP(uint256 assets, address entryToken) public  returns(uint256) {
        // ONLY ACCEPTS ERC20. If you want to enter with ether, you need to wrap first.
        _distributeReward(_msgSender());
        IERC20MetadataUpgradeable(entryToken).safeTransferFrom(_msgSender(), address(this), assets);
        if (entryToken == address(wETH)) {
        ...
        if (entryToken == poolTokens.at(0)) {
            assets = ICurvePool(curvePool).add_liquidity{value:assets}([assets, 0], 0);
        } else {
    ...
    function withdrawToNonLp(
        ...
        if (exitToken == poolTokens.at(0)) {
            ...
            require(success, "Error when sending");
        } else if (exitToken == poolTokens.at(1)) {
            ...
            shares = ICurvePool(curvePool).remove_liquidity_one_coin(shares, 0, 0);
            shares = exchange.exchange{value:shares}(poolTokens.at(0), exitToken, shares,0);
            IERC20MetadataUpgradeable(exitToken).safeTransfer(receiver, shares);
        }
        return shares;
    }
